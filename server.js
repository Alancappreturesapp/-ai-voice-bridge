...
wss.on('connection', (twilioWs, req) => {
  ...
  aiWs.on('open', () => {
    ...
    aiWs.send(JSON.stringify({ type: 'session.update', session: {
      ...
      voice: 'alloy', modalities: ['text','audio'], temperature: 0.7,
...
async function book(text, p) {
  ...
  await fetch(BASE44_WEBHOOK_URL, {
    ...
    body: JSON.stringify({ contact_id: p.contact_id, contact_name: p.contact_name, agent_name: p.agent_name,
